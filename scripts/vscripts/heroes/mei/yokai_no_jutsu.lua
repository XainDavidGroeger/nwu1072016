...
function applyDamage( keys )
	local slow_percent_per_stack = keys.ability:GetLevelSpecialValueFor("slow_percent_per_stack", (keys.ability:GetLevel() - 1))
	local max_debuff_stack = keys.ability:GetLevelSpecialValueFor("slow_percent_per_stack", (keys.ability:GetLevel() - 1))
	...
	if not keys.target:IsBuilding() then
		...
		local damage = keys.damageAmount
	...
	for i = 0, keys.target:GetModifierCount() - 1 do
		local modifier = keys.target:FindModifierByName(keys.target:GetModifierNameByIndex(i))

		local caster_key = TableFindKey(GameRules.debuffModifiers, modifier:GetName())
		if caster_keys ~= nil then
			debuff_count = debuff_count + 1
		end
	end

	if debuff_count > max_debuff_stack then
		debuff_count = max_debuff_stack
	end

	keys.ability:ApplyDataDrivenModifier(keys.caster,keys.target,"modifier_mei_yokai_ms_slow",{duration = duration})
	keys.target:SetModifierStackCount("modifier_mei_yokai_ms_slow",keys.ability, (slow_percent_per_stack * debuff_count))
