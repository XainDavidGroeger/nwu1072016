...
function anko_giant_snake:OnSpellStart()
	...
	ParticleManager:SetParticleControlEnt( self.nChainParticleFXIndex, 0, self:GetCaster(), PATTACH_POINT_FOLLOW, "attach_hitloc", self:GetCaster():GetOrigin() + self.vHookOffset, true )
...
function anko_giant_snake:OnProjectileHit( hTarget, vLocation )
	...
	if self.bRetracting == false then
		...
		if hTarget ~= nil and ( not hTarget:IsInvisible() ) and bTargetPulled then
			...
			ParticleManager:SetParticleControlEnt( self.nChainParticleFXIndex, 1, self:GetCaster(), PATTACH_POINT_FOLLOW, "attach_hitloc", self:GetCaster():GetOrigin() + self.vHookOffset, true);
		end
