...
function internal_bleeding( keys )
	...
	local bleeding_mana_burn = ability:GetLevelSpecialValueFor("manaburn_dot", (ability:GetLevel() - 1))
	...
	if (target:GetMana() - bleeding_mana_burn) > 0 then
		target:SetMana(target:GetMana() - bleeding_mana_burn)
	else
		target:SetMana(1)
	end
