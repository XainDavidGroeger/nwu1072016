...
function wood_release( keys )
	...
	for i = 1,tree_count do
			...
			local nearbyUnits = FindUnitsInRadius(keys.caster:GetTeamNumber(), Vector( posX, posY, 0.0 ), nil, 70, DOTA_UNIT_TARGET_TEAM_BOTH, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, 0, FIND_ANY_ORDER, false)
			if nearbyUnits then
				for _,unit in pairs(nearbyUnits) do
					FindClearSpaceForUnit(unit, target_point, false)
				end
			end
